function(task, responses){
    if(task.status.includes("error")){
        ...
        for(let i = 0; i < responses.length; i++)
        {
            ...
            tableHeader = "Contents of " + data[0]["path"].substr(0, data[0]["path"].lastIndexOf("\\"));
            for(let j = 0; j < data.length; j++){
                ...
                let startIconColor = "";
                if (finfo["Isfile"]) {
                    var fileExt = "." + finfo['path'].split(".").slice(-1)[0].toLowerCase();
                    ...
                    let cat_parameters = "";
                    if (finfo["path"].includes(":")) {
                        cat_parameters = finfo["path"];
                    } else {
                        cat_parameters = finfo["path"];
                    }
                    buttonSettings = {
...
data["files"][j]["creation_date"], "cellStyle": {}},
                ...
                let creation_date = new Date(data[j]["creationTime"]);
                let last_modified_date = new Date(data[j]["lastWriteTime"]);
                let access_date = new Date(data[j]["lastAccessTime"]);

                let fmt_creation = creation_date.getDate() + "/" + (creation_date.getMonth() + 1) + "/" + creation_date.getFullYear()
                let fmt_last_modified = last_modified_date.getDate() + "/" + (last_modified_date.getMonth() + 1) + "/" + last_modified_date.getFullYear()
                let fmt_access = access_date.getDate() + "/" + (access_date.getMonth() + 1) + "/" + access_date.getFullYear()
                /*let acl_rows = [];
                ...
                let row = {
                    ...
                    "Task": buttonSettings,
                    "name": {
                        "plaintext": data[j]["path"].split("\\").slice(-1)[0],
                        "cellStyle": {},
                        "startIcon": startIcon,
                        "startIconHoverText": startIconHoverText,
                        "startIconColor": startIconColor
                    },
                    "size": {"plaintext": data[j]["size"], "cellStyle": {}},
                    //"owner": {"plaintext": data[j]["owner"], "cellStyle": {}},
